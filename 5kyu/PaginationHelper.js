...
The types of values contained within the collection/array are
not relevant.
...
class PaginationHelper {
  ...
  pageItemCount(pageIndex) {
    ...
    const start = pageIndex * this._itemsPerPage;
    const end = this._itemsPerPage * (pageIndex + 1);
    const result = this._collection.slice(start, end);
  ...
  pageIndex(itemIndex) {
    ...
    if(itemIndex >= this._collection.length || itemIndex < 0) return -1;
