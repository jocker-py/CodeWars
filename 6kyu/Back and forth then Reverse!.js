/*
link: https://www.codewars.com/kata/60cc93db4ab0ae0026761232/javascript
...
function arrange(s) {
  ...
  for(let i = 0; i < Math.ceil(s.length / 2); i++){
    const first = i % 2 === 0 ? s[i] : s[s.length - i - 1];
    const last = i % 2 !== 0 ? s[i] : s[s.length - i - 1];
    if(first !== undefined) t.push(first);
    if(last !== undefined && i !== s.length - i - 1) t.push(last);
  }
  return t;
}